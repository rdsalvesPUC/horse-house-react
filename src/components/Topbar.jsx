...
export default function Topbar({harasList, choseHaras, userData, disableSelect}) {
    const [dropdown, setDropdown] = useState(false)
    const navigate = useNavigate();

    return (
        <header>
            <div className={`h-16 border-b border-gray-200 bg-white flex items-center ${disableSelect ? "justify-end" : "justify-between"} px-6`}>
                ...
                {!disableSelect &&
                    <div className="flex-1 max-w-md px-6">
                        <div className="relative">
                            ...
                            <input type="text" placeholder="Buscar..." className="w-full pl-10 pr-4 py-2 border border-secondary/50 rounded-md focus:outline-none"/>
                        </div>
                    </div>}
